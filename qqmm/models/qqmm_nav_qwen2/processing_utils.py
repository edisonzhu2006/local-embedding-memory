...
class BaseVLProcessor(BaseProcessor):
    ...
    # Handle transformers 5.0.0+ where optional_attributes may not exist
    optional_attributes = getattr(BaseProcessor, "optional_attributes", []) + ["image_token_len", "vision_token_share_pe"]
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, image_processor: BaseImageProcessor,
        ...
        self.vision_token_share_pe = vision_token_share_pe
        super().__init__(tokenizer=tokenizer, image_processor=image_processor, **kwargs)

        # Add image token as special token (transformers 5.0.0+ compatibility)
