...
class FunctionsWrapper(BaseChatModel, BaseLanguageModel):
    ...
    response_logs: List = []
    def __init__(self, llm, debug=False, temperature=0.0001) -> None:
        super().__init__()
    ...
    def _generate_with_raise(
        ...
        **kwargs: Any,
        ...
        if self.debug:
            ...
            self.response_logs.append(chat_generation_content)
        ...
        try:
            if "```json" in chat_generation_content:
                # print("Detected ```json block in response, attempting to parse content within the block.")
                cleaned_json = json_fixer(chat_generation_content)
                parser = JsonOutputParser()
                parsed_chat_result = parser.invoke(cleaned_json)
                # chat_generation_content = chat_generation_content.replace("\'", "\"")
            else:
                # print("No ```json block detected, attempting to parse full response.")
                parsed_chat_result = json.loads(chat_generation_content)
            if parsed_chat_result is None:
                print(chat_generation_content)
                raise ValueError("Parsed chat result is None.")
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        try:
            return self._generate_with_raise(
            ...
            if self.temperature > 0.5:
                ...
                if self.debug:
                    print("Met ERROR when generating the deterministic response... \n" + str(e) + "\nUsing null as response...")
                return ChatResult(
                    generations=[
                        ChatGeneration(
                            message=AIMessage(
