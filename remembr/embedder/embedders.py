    ...
    def __init__(self,
        ...
        if backend == "oc":
            ...
            if "paligemma" in hf_model_id:
                ...
                config_qqmm.merge_from_yaml('./configs/embed/qqmm-embed/mmeb.yaml')
                ...
                if quant == True:
                    print("Using 4-bit quantization for QQMM")
                    try:
    ...
    def encode_image(self, frame_paths):
        ...
        with torch.no_grad():
            for i in tqdm(range(0, len(frame_paths), self.batch_size), desc="Embed images", leave=False):
                ...
                if self.backend == "oc":
                    # ims = [self.preprocess(safe_image_open(p)) for p in batch_paths]
                    # import ipdb; ipdb.set_trace()
                    ...
                    if "paligemma" in self.hf_model_id:
                        ...
                        # Batch process all images at once for better performance
                        img_feat = torch.cat([
                            self.model.chat(text='Represent the given image for classification.', image=[image])
                            for image in images
                        ], dim=0)
                        img_feat = img_feat.to(torch.float32)
                        # 3584-dim feature embeddings
                    else:
                        raise NotImplementedError
