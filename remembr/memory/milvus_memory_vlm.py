...
class MilvusVLMWrapper:

    def __init__(self, collection_name='test', ip_address='127.0.0.1', port=19530, drop_collection=False, dim=1024):
        self.dim = dim
        self.collection_name = collection_name
        self.collection = self.connect_to_milvus_collection(collection_name, dim, address=ip_address, port=port, drop_collection=drop_collection)
    ...
    def connect_to_milvus_collection(self, collection_name, dim, address='127.0.0.1', port=19530, drop_collection=False):
        ...
        fields = [
            ...
            FieldSchema(name='vlm_embedding', dtype=DataType.FLOAT_VECTOR, description='embedding vectors', dim=dim),
            ...
            FieldSchema(name='image_file_path', dtype=DataType.VARCHAR, description='image file path string', max_length=3000),
        ]
        ...
        collection.create_index(field_name="vlm_embedding", index_params=index_params)
    ...
    def search(self, data):
        ...
        res = self.collection.search(
            data=[data],
            anns_field="vlm_embedding",
            param=param,
            batch_size=BATCH_SIZE,
            limit=LIMIT,
            # expr="id > 3",
            output_fields=["id", "vlm_embedding"]
        )
...
class MilvusVLMMemory(Memory):


    def __init__(self, db_collection_name: str, embedder: VLMEmbeddings, db_ip='127.0.0.1', db_port=19530, time_offset=FIXED_SUBTRACT, dim=1024, retriever_k=5):
        self.dim = dim
    ...
    def insert(self, item: MemoryItem, vlm_embedding=None):
    ...
    def reset(self, drop_collection=True):
        ...
        self.milv_wrapper = MilvusVLMWrapper(self.db_collection_name, self.db_ip, self.db_port, drop_collection=drop_collection, dim=self.dim)

        text_vector_db = Milvus(
            self.embedder,
            connection_args={"host": self.db_ip, "port": self.db_port},
            collection_name=self.db_collection_name,
            vector_field='vlm_embedding',
            text_field='caption',
        )
        self.text_retriever = text_vector_db.as_retriever(search_kwargs={"k": self.retriever_k})
    ...
    def search_by_text(self, query: str) -> str:
        ...
        docs = self.text_retriever.invoke("[TXT]" + query)
    ...
    def memory_to_string_vlm(self, memory_list: list[Document], ref_time: float=None):
        if ref_time == None:
        ...
        for doc in memory_list:
            ...
            s += f"The robot saw the following: [IMG]{doc.metadata['image_file_path']}[/IMG]\n\n"
            out_string += s
        return out_string
