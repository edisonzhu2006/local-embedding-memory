...
from remembr.utils.util import file_to_string, json_fixer
...
DEFAULT_RESPONSE_TEMPLATE = """```json
{
...
#     type: Literal["text", "position", "time", "duration", "binary"]
#     text: str | None = None
...
# json_parser = JsonOutputToolsParser(pydantic_object=Answer)
...
class ReMEmbRAgent(Agent):
    ...
    def __init__(self, llm_type='gpt-4o', num_ctx=8192, temperature=0, debug=False, prompt_dir="unknown"):
        ...
        # print("ReMEmbRAgent using HuggingFace text embedding")

        # self.update_for_instance() # ref_time is None this time
        

        self.prompt_selector(llm_type)
    ...
    def prompt_selector(self, llm_type):
    ...
    def llm_selector(self, llm_type, temperature, num_ctx):
        ...
        # elif 'paligemma' in llm_type:
        #     from transformers import Qwen2_5_VLForConditionalGeneration
        #     # model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
        #     #     llm_type, torch_dtype="auto", device_map="auto"
        #     # )
        #     # tokenizer = AutoTokenizer.from_pretrained(llm_type)
        #     # model = AutoModelForCausalLM.from_pretrained(llm_type)
        #     # pipe = pipeline("text-generation", model=model, tokenizer=tokenizer, max_new_tokens=num_ctx, temperature=temperature)
        #     # llm = HuggingFacePipeline(pipeline=pipe)
        #     llm = HuggingFacePipeline.from_model_id(
        #         model_id=llm_type,
        #         task="text-generation",
        #         pipeline_kwargs={"max_new_tokens": num_ctx, "temperature": temperature},
        #     )
        ...
        # Qwen for Caption Only
        elif 'qwen2.5' in llm_type.lower() and 'vl' not in llm_type.lower():
            llm = ChatOllama(
    ...
    def agent_msg_decorator(self, messages):
        ...
        self.retrieval_logs.append([str(message) for message in messages])
    ...
    def agent(self, state):
        ...
        messages = self.agent_msg_decorator(messages)

        if self.debug:
            print("\n=======================Agent Call=======================")
            print({"question": question, "chat_history": [type(msg) for msg in messages]})
            for msg in messages:
                if type(msg) == ToolMessage:
                    print("Tool:", str(msg.content)[:100])
                elif type(msg) == AIMessage:
                    print("AI:", str(msg.content)[:100])
                elif type(msg) == HumanMessage:
                    if isinstance(msg.content, list):
                        print("Human:", [x['type'] for x in msg.content])
                    else:
                        print("Human:", str(msg.content)[:100])
        ...
        if self.debug:
            print("\nResponse:", response)
            print("========================================================")
        if self.llm_response_count < len(self.chat.response_logs):
            self.retrieval_logs.append(self.chat.response_logs[self.llm_response_count:])
            self.llm_response_count = len(self.chat.response_logs)
        ...
        if response.tool_calls:
            ...
            if self.temperature < 0.5 and not response.content:
                # invalid response, calling generate directly
    ...
    def generate(self, state):
        ...
        # otherwise use this sentence
        model = gen_prompt | self.chat

        # if eval NavQA use ollama qwen2.5-vl-3b-instruct model, use sentence following
        # model = gen_prompt | self.chat.llm

        chat_history = self.gen_msg_decorator(messages)
        ...
        if self.debug:
            print("\nResponse:", response)
            print("===========================================================")
        self.retrieval_logs.append(response)
        ...
        try:
            ...
            if self.temperature > 0.5:
                ...
                parsed = parse_gen_json(DEFAULT_RESPONSE_TEMPLATE)
