...
def load_position_csv(csv_path: str) -> Dict[int, List[float]]:
    """
    Load position data from a CSV file with columns: frame_index,position_x,position_y,position_z,...
    Returns dict mapping frame_index -> [x, y, z].
    """
    import csv
    index_to_pos = {}
...
def find_position_csv(vdir: str, data_root: str) -> Optional[str]:
    """
    Search for a position CSV file matching the video directory.
    Looks for files like position_data_*.csv in the project root and data directories.
    """
    import glob as globmod
    vid = os.path.basename(vdir)
    # Search in common locations
    search_dirs = [vdir, data_root, os.path.dirname(data_root), "."]
    for d in search_dirs:
        for pattern in [f"position_data*{vid}*.csv", f"position_data*.csv", f"*{vid}*position*.csv"]:
...
def main():
    ...
    ap.add_argument("--position_csv", type=str, default=None, help="Path to CSV with per-frame positions (columns: frame_index,position_x,position_y,position_z)")
    args = ap.parse_args()
    ...
    for vdir in video_dirs:
        ...
        # Try to load position data from CSV
        csv_path = args.position_csv if hasattr(args, 'position_csv') and args.position_csv else find_position_csv(vdir, data_root)
        index_to_pos = {}
        if csv_path:
            index_to_pos = load_position_csv(csv_path)
            print(f"  Loaded {len(index_to_pos)} frame positions from {csv_path}")
        ...
        if frame_files:
            ...
            for idx, fname in enumerate(frame_files, 1):
                ...
                # If no direct match, look up from position CSV by frame index
                if position is None and index_to_pos:
                    frame_idx = idx - 1  # 0-based index
                    if frame_idx in index_to_pos:
                        position = index_to_pos[frame_idx]
                        n_interpolated += 1
            ...
            if n_interpolated > 0:
                print(f"  Loaded positions from CSV for {n_interpolated}/{num_frames} frames")
